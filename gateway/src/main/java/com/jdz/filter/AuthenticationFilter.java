...
import com.jdz.config.Carousel;
import com.jdz.utils.JwtUtils;
...
public class AuthenticationFilter extends AbstractGatewayFilterFactory<AuthenticationFilter.Config> {
    ...
    public AuthenticationFilter(RouteValidator validator, RestTemplate template, JwtUtils jwtUtils, Carousel carousel) {
        super(Config.class);
            ...
            if (validator.isSecure.test(exchange.getRequest())) {
                if (!exchange.getRequest().getCookies().containsKey(HttpHeaders.AUTHORIZATION) && !exchange.getRequest().getCookies().containsKey("refresh")) {
                    ...
                    return exchange.getResponse().writeWith(
                            Mono.just(exchange.getResponse().bufferFactory().wrap(sb.toString().getBytes())));
                }
                ...
                try {
                    if (activeProfile.equals("test")) {
                        ...
                        if (response.getStatusCode() == HttpStatus.OK) {
                            ...
                            if (cookieList != null) {
                                List<java.net.HttpCookie> httpCookies = java.net.HttpCookie.parse(cookieList.getFirst());
                    ...
                    return exchange.getResponse()
                            .writeWith(Flux.just(new DefaultDataBufferFactory().wrap(message.getBytes())));
                }
            }
